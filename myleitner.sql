...
CREATE TABLE `card` (
  ...
  `LEITNER_ID` int(11) DEFAULT NULL,
  `TITLE` text,
  `FRONT` text,
  `BACK` text,
  ...
  KEY `FK_LEITNER_CARD` (`LEITNER_ID`),
  CONSTRAINT `FK_LEITNER_CARD` FOREIGN KEY (`LEITNER_ID`) REFERENCES `leitner` (`ID`) ON DELETE SET NULL ON UPDATE SET NULL,
...
CREATE TABLE `comment` (
  ...
  `LEITNER_ID` int(11) NOT NULL,
  `CONTEXT` text,
  `CREATE_TIME` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `LIKE_COUNT` int(11) DEFAULT NULL,
  `DISLIKE_COUNT` int(11) DEFAULT NULL,
  PRIMARY KEY (`ID`),
  KEY `FK_CARD_COMMENT` (`CARD_ID`),
  KEY `FK_LEITNER_COMMENT` (`LEITNER_ID`),
  KEY `FK_USER_COMMENT` (`USER_ID`),
  CONSTRAINT `FK_CARD_COMMENT` FOREIGN KEY (`CARD_ID`) REFERENCES `card` (`ID`),
  CONSTRAINT `FK_LEITNER_COMMENT` FOREIGN KEY (`LEITNER_ID`) REFERENCES `leitner` (`ID`),
...
CREATE TABLE `leitner` (
  ...
  `CREATE_TIME` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`ID`),
  KEY `FK_LEITNER_USER` (`USER_ID`),
  CONSTRAINT `FK_LEITNER_USER` FOREIGN KEY (`USER_ID`) REFERENCES `user` (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
